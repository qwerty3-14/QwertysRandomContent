...
namespace QwertysRandomContent.NPCs.HydraBoss
{
	public class Head5 : ModNPC
	{
        ...
        public override void AI()
		{
			...
			if(npc.rotation > MathHelper.ToRadians(360))
			{
				npc.rotation-=MathHelper.ToRadians(360);
			}
			if(npc.rotation < MathHelper.ToRadians(0))
			{
				npc.rotation+=MathHelper.ToRadians(360);
			}
           ...
           // npc.realLife = (int)npc.ai[0];
            ...
            if (npc.ai[3] == 2)
			{
                ...
                {
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, 0f, 10f, mod.ProjectileType("HydraBreath"), (int)(damage * .8f), 3f, Main.myPlayer);
                }
                if (varTime >= 60)
				{
                    if (Main.netMode != 1)
            ...
            Vector2 moveTo = new Vector2(Hydra.Center.X-(50+npc.ai[1]), Hydra.Center.Y-(300f+npc.ai[2]))- npc.Center;	
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            if (Main.netMode != 0)
            {
                ...
                spriteBatch.Draw(mod.GetTexture("NPCs/HydraBoss/HydraNeckBase"), neckOrigin - Main.screenPosition,
                    ...
                    spriteBatch.Draw(mod.GetTexture("NPCs/HydraBoss/Head1"), new Vector2(npc.Center.X - Main.screenPosition.X, npc.Center.Y - Main.screenPosition.Y),
                            npc.frame, drawColor, npc.rotation,
                            new Vector2(106 * 0.5f, 72 * 0.5f), 1f, SpriteEffects.None, 0f);
                    spriteBatch.Draw(mod.GetTexture("NPCs/HydraBoss/Head1_Glow"), new Vector2(npc.Center.X - Main.screenPosition.X, npc.Center.Y - Main.screenPosition.Y),
		...
		public override void BossHeadSlot(ref int index)
		{
				...
				index = NPCHeadLoader.GetBossHeadSlot(QwertysRandomContent.Head1Head);
